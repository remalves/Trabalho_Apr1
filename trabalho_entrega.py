    ...
    while continuar:
        ...
        if opcao == 1:
           if len(dic_clientes) == 0:
            ...
            print('\nVoltando ao menu principal e gravando os dados ...')
            gravar_dados_clientes(clientes_arq, dic_clientes)
...
def submenu_produtos(produtos_arq, dic_produtos):
    ...
    while continuar:
        ...
        if opcao == 1:
            ...
            codigo = input('\nDigite o código do novo produto: ').strip()
            ...
            print('\nVoltando ao menu inicial e gravando os dados ...')
            gravar_dados_produtos(produtos_arq, dic_produtos)
            return False
        else:
            print('\nOpção inválida. Digite um número de 1 a 6')

''' ------------------------------------------------------------------------------ Compra/Venda -------------------------------------------------------------------------------------------------------------------'''
def listar_todas_compras(cpf_cliente, dic_compras):
    encontrou = False
...
def alterar_compra(chave, dic_compras):
    if chave not in dic_compras:
        ...
        novo_desconto_valido = False
        while not novo_desconto_valido:
            try:
                novo_desconto = float(input('\nDigite o valor do novo desconto: '))
                novo_desconto_valido = True
            except:
                print('\nDigite um valor válido!')
                novo_desconto_valido = False 

        novo_valor_desconto = (novo_preco * novo_desconto) / 100
        novo_valor_final = novo_preco - novo_valor_desconto
        dic_compras[chave] = novo_valor_final
        print('\nCompra alterada com sucesso!')
        
            
def excluir_compra(chave,dic_compras):
  if chave not in dic_compras:
    print('\nCompra não cadastrada!')
  else:
    del dic_compras[chave]
    ...
    while continuar:
        ...
        if opcao == 1:
            if len(dic_compras) == 0:
                ...
                print('\nListando todas as compras de um cliente...')
            ...
            if len(dic_compras) == 0:
                ...
                print('\nConsultando uma compra específica...')
                cpf = input('Digite o CPF do cliente: ').strip()
            ...
            if len(dic_compras) == 0:
                ...
                print('\nAlterando compra...')
                cpf = input('Digite o CPF do cliente: ').strip()
                tem_compras = listar_todas_compras(cpf, dic_compras)
                if tem_compras:
                    print('\nDigite os demais dados para alterar a compra desejada:')
                    codigo = input('Digite o código do produto: ').strip()
                    ...
                    print('\nOperação cancelada. Nenhuma compra para este cliente.')
            ...
            print('\nGravando os dados e voltando ao menu principal ...')
            gravar_dados_compras(compras_arq, dic_compras)
            continuar = False
...
def relatorio_clientes_telefone(qtd,dic_clientes, clientes_telefones_arq):
    ...
    if len(dic_clientes)==0:
        ...
        # Retorna True se teve cliente gravado, False se nenhum cliente atendeu o critério
        return gravar_relatorio_clientes_telefone_em_arquivo(qtd, dic_clientes, clientes_telefones_arq)
        
def relatorio_produtos_vencidos(produtos_vencidos_arq,data_atual, dic_produtos):
   if len(dic_produtos)==0:
        return False
   else:
       gravar_relatorio_produtos_vencidos_em_arquivo(produtos_vencidos_arq,data_atual, dic_produtos)
       return True

def relatorio_vendas_periodo(data_inicial, data_final, dic_compras,vendas_periodos_arq):
       gravou = gravar_relatorio_vendas_periodo_em_arquivo(data_inicial, data_final, dic_compras,vendas_periodos_arq)
       if gravou:
           return True
       else:
           return False
    ...
    while continuar:
        ...
        while not opcao_valida:
            ...
            if opcao == 1:
                ...
                gerou = relatorio_clientes_telefone(qtde, dic_clientes, clientes_telefones_arq)
                if gerou:
                    print('\nRelatório gerado com sucesso!')
                else:
                    print('\nNenhum cliente encontrado com mais de', qtde, 'telefones ou lista de clientes está vazia.')


            elif opcao == 2:
                data_atual = input('\nData atual (dd/mm/aaaa): ')
                res = relatorio_produtos_vencidos(produtos_vencidos_arq,data_atual, dic_produtos)
                if res:
                    print('\nArquivo do relatório gerado com sucesso!')
                else:
                    print('\nErro ao gerar arquivo')
                    
            elif opcao ==3:
                print('\nConsultando vendas entre duas datas')
                data_ini = input('Digite a data incial: ')
                data_fim = input("Digite a data final: ")
                gerou_relatorio = relatorio_vendas_periodo(data_ini, data_fim,dic_compras,vendas_periodos_arq)
                if gerou_relatorio:
                    print('\nArquivo do relatório gerado com sucesso!')
                else:
                    print('\nErro ao gerar arquivo')        
...
#---------------------------------------------------------------------------------------------- Clientes -------------------------------------------------------------------------------------------#
def gravar_dados_clientes(clientes_arq, dic_clientes):
...
#------------------------------------------------------------------------------------------ Produtos -----------------------------------------------------------------------------------------#
def gravar_dados_produtos(produtos_arq,dic_produtos):
...
def carregar_dados_produtos(produtos_arq):
    ...
    if existe_arquivo(produtos_arq):
        ...
        for linha in arq_produtos:
            ...
            desconto = float(campos[4])
            data_validade = campos[5]
            dic_produtos[codigo] = [descricao, peso, preco, desconto, data_validade]
        arq_produtos.close()
    return dic_produtos

# ------------------------------------------------------------------------------- Compra/Venda -----------------------------------------------------------------------------------------------------------------#
def gravar_dados_compras(compras_arq,dic_compras):
...
def gravar_relatorio_produtos_vencidos_em_arquivo(produtos_vencidos_arq, data_atual, dic_produtos):
...
def gravar_relatorio_vendas_periodo_em_arquivo(data_ini, data_fim, dic_compras, vendas_periodos_arq):
    try:
        ...
        arq.close()
        return vendas_encontradas # Retorna True se teve alguma venda, False se não teve
    except ValueError:
        return False 
